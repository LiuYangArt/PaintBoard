...
| ------------------------ | ----------- | ---------- | --------------- |
| **Avg FPS**              | 60.0        | 59.9       | ✅ 正常         |
| **P99 Frame**            | 18.0ms      | 17.9ms     | ⚠️ 略超 16.6ms  |
| **Input Latency**        | 5.56ms      | 5.3ms      | ✅ 正常         |
| **Render Latency (Avg)** | **20.78ms** | 7.0ms      | 🚨 App 严重     |
| **Render Latency (P99)** | **27.30ms** | 8.5ms      | 🚨 App 非常严重 |
| **Max Visual Lag**       | **713px**   | **1990px** | 🚨 两者都严重   |

> ⚠️ 注意：自动化 Benchmark 在 App 中的采样存在问题（合成事件未正确触发），上述 App 数据来自实时面板截图。
